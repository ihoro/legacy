...
type
  ...
  TMain = class(TForm)
    ...
    N3: TMenuItem;
    mmiImport: TMenuItem;
    opdImport: TOpenPictureDialog;
    tobImport: TToolButton;
    ilHelp: TImageList;
    N6: TMenuItem;
    N7: TMenuItem;
    ...
    procedure mmiImportClick(Sender: TObject);
    procedure tobImportClick(Sender: TObject);
    ...
    with Action do
      case Action.ActionType of
        atPoint:begin
          ...
          if PenWidth=1 then
            if PutDel then
              Pixels[X1-FCenterX,Y1-FCenterY+CoolBar1.Height]:=clMainColor
                      else
              Pixels[X1-FCenterX,Y1-FCenterY+CoolBar1.Height]:=clWhite
                        else
            begin
    ...
    begin
      ...
      for FL:=1 to FRegionPicture.TotalActions do
        begin
          ...
          {Rgn:=WriteAction(FTempAction);
          if FL=1 then
            MainRegion:=Rgn
                  else
            if FTempAction.PutDel then
              CombineRgn(MainRegion,MainRegion,Rgn,RGN_OR)
                                  else
              CombineRgn(MainRegion,MainRegion,Rgn,RGN_DIFF);}
      ...
      {writeln(FFileCode,
        '  SetWindowRgn(Handle,',IntToStr(MainRegion),',false)');}
      CloseCode
    end
end;
...
begin
  ...
  if FRegionPicture.TotalActions>1 then
    writeln(FFileCode,'  Region: THandle;');
  for f:=1 to FRegionPicture.TotalActions do
    begin
      seek(FTempFile,f-1);
      read(FTempFile,FTempAction);
      if FTempAction.ActionType=atLine then
        begin
...
var
  s, X1s, Y1s, X2s, Y2s,
  PenWidths,  Xdiv2, Ydiv2: string;
  delX, delY: Integer;
  k: Real;
  {ForPen,ForPen1,ForPen2: THandle;
    ...
    case ActionType of
      atPoint:begin
        if PenWidth=1 then
          begin
            ...
            Y2s:=IntToStr(Y1+1);
            writeln(FFileCode,'  '+
              s+':=CreateRectRgn('+X1s+','+Y1s+
              ','+X2s+','+Y2s+');');
            {X2s:=IntToStr(X1+3);
            Y2s:=IntToStr(Y1+3);
            writeln(FFileCode,'  '+
              'ForPen:=CreateRectRgn('+X1s+','+Y1s+
              ','+X2s+','+Y2s+');');
            X1s:=IntToStr(X1+1);
            writeln(FFileCode,'  '+
              'ForPen1:=CreateRectRgn('+X1s+','+Y1s+
              ','+X2s+','+Y2s+');');
            X1s:=IntToStr(X1);
            Y1s:=IntToStr(Y1+1);
            writeln(FFileCode,'  '+
              'ForPen2:=CreateRectRgn('+X1s+','+Y1s+
              ','+X2s+','+Y2s+');');
            writeln(FFileCode,'  CombineRgn('+
              'ForPen1,ForPen1,ForPen2,RGN_OR);');
            writeln(FFileCode,'  CombineRgn('+
              s+',ForPen,ForPen1,RGN_DIFF);');}
            {Combines}
            {writeln(FFileCode,'  '+s+
              ':=',IntToStr(ForPen),';')
            Result:=ForPen}
          ...
          begin
            ...
            writeln(FFileCode,'  '+s+
              ...
              +Y1s+','+X2s+','+Y2s+');')
            {ForPen:=CreateEllipticRgn(
              X1-(PenWidth div 2),Y1-(PenWidth div 2),
              X1+(PenWidth div 2),Y1+(PenWidth div 2));
            Result:=ForPen}
          end
      end;
      atLine:begin
        ...
        writeln(FFileCode,'  Region'+
          ':=CreateEllipticRgn('+X1s+','
          +Y1s+','+X2s+','+Y2s+');');
        {ForPen2:=CreateEllipticRgn(
          X2-(PenWidth div 2),Y2-(PenWidth div 2),
          X2+(PenWidth div 2),Y2+(PenWidth div 2));
        CombineRgn(ForPen1,ForPen1,ForPen2,RGN_OR);}
        FTempAction.PutDel:=PutDel;
        CombineActions(FTempAction);
        ...
        {ForPen2:=CreatePolygonRgn(Points,4,ALTERNATE);
        CombineRgn(ForPen1,ForPen1,ForPen2,RGN_OR);
        Result:=ForPen1}
      end;
      atRect:begin
        ...
        {ForPen:=CreateRectRgn(X1,Y1,X2,Y2);
        Result:=ForPen}
      end;
      atEllipse:begin
        writeln(FFileCode,'  '+s+':='+
          'CreateEllipticRgn('+X1s+','+Y1s+','+X2s+','+Y2s+');');
        {ForPen:=CreateEllipticRgn(X1,Y1,X2,Y2);
        Result:=ForPen}
      end;
      atRoundRect:begin
        writeln(FFileCode,'  '+s+':='+
          ...
          Xdiv2+','+Ydiv2+');')
        {ForPen:=CreateRoundRectRgn(X1,Y1,X2,Y2,
          ABS(X2-X1) div 2,ABS(Y2-Y1) div 2);
        Result:=ForPen}
      end
    end
end;
...
procedure TMain.mmiImportClick(Sender: TObject);
var
  TrColor: TColor;
  f,l,iX,iY: Integer;
begin
  ...
  with TransColor do
    begin
      ShowModal;
      TrColor:=Shape1.Brush.Color;
      iX:=SpinEdit1.Value;
      iY:=SpinEdit2.Value
    end;
  FImportPic:=TBitmap.Create;
  FImportPic.LoadFromFile(opdImport.FileName);
  tobUndo.Enabled:=true;
  mmiUndo.Enabled:=true;
  FSaved:=false;
  with FCurrentAction do
    begin
      ...
      X1:=iX;
      Y1:=iY;
      X2:=iX+FImportPic.Width-1;
      Y2:=iY+FImportPic.Height-1
    end;
  SaveAction(FCurrentAction);
  for l:=0 to FImportPic.Height-1 do
    for f:=0 to FImportPic.Width-1 do
      if FImportPic.Canvas.Pixels[f,l]=TrColor then
        begin
          with FCurrentAction do
            begin
              ActionType:=atPoint;
...
procedure TMain.tobImportClick(Sender: TObject);
begin
  mmiImportClick(nil)
end;
