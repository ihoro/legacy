...
begin
  ...
  RegSetup.WriteString('JAP',OpenJAP);
...
begin
  if not RegSetup.OpenKey('\SOFTWARE\OIVSoft\Japanese Crossword',false) then
    begin
      RegSetup.OpenKey('\SOFTWARE\OIVSoft\Japanese Crossword',true);
      RegSetup.WriteInteger('Background',ColorToRGB(ccTitle));
      RegSetup.WriteInteger('Point',ColorToRGB(ccPoint));
      RegSetup.WriteInteger('BackgroundFigures',ColorToRGB(ccFonFigures));
      RegSetup.WriteInteger('Figures',ColorToRGB(ccFigures));
      ...
      RegSetup.WriteString('JAP',TekDir);
    ...
    begin
      ...
      OpenJAP:=RegSetup.ReadString('JAP');
...
Rewrite(Jap);
writeln(Jap,CoderStr(OIVSoft,Key));
writeln(Jap,CoderStr(ToStr(Crossword.CrosswordX),Key));
writeln(Jap,CoderStr(ToStr(Crossword.CrosswordY),Key));
for l:=1 to Crossword.CrosswordY do
  for f:=1 to Crossword.CrosswordX do
    begin
      if (l=Crossword.CrosswordY) and (f=Crossword.CrosswordX) then
        writeln(Jap,CoderStr(BoToStr(Crossw[f,l]),Key))
                               else
        write(Jap,CoderStr(BoToStr(Crossw[f,l]),Key))
    end;
writeln(Jap,CoderStr(OIVSoft,Key));
except
WriteError:=true;
...
if LowerCase(ExtractFileExt(FParams))='.jap' then
  begin
    AutoLoad:=true;
    AutoLoadFileName:=FParams;
    MainMode:=false;
