comment `#####################################################
#
#  ffc.asm    21.03.2006 by fnt0m32 'at' gmail.com
#
#  FF Catcher v0.9.7
#
#  Used [MASM32 v8.2]
...
DlgProc proc uses ebx esi edi hwnd:HWND,msg:UINT,wParam:WPARAM,lParam:LPARAM
	...
	@@:
		...
		shr		eax,1							; CX / 2
		mov		ebx,_wnd_rect.right				; DCX of main window (minimum)
		shr		ebx,1							; DCX / 2
		sub		eax,ebx							; CX / 2  -  DCX / 2
		mov		_wnd_rect.left,eax				; default left for center
		invoke	GetSystemMetrics,SM_CYSCREEN
		shr		eax,1							; CY / 2
		mov		ebx,_wnd_rect.bottom			; DCY of main window (minimum)
		shr		ebx,1							; DCY / 2
		sub		eax,ebx							; CY / 2  -  DCY / 2
...
@@:
	...
	; check if position/size is zero (it can be with old config) - checks by .left only ! it's enough
	cmp		cfg.wnd_rect.left,0
	jnz		@F
	; else set defaults
	push	_wnd_rect.left
