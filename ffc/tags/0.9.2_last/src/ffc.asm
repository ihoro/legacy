comment `#####################################################
#
#  ffc.asm    06.02.2006 by fnt0m32 'at' gmail.com
#
#  FF Catcher v0.9.2
#
#  Used [MASM32 v8.2]
...
DlgProc proc uses ebx esi edi hwnd:HWND,msg:UINT,wParam:WPARAM,lParam:LPARAM
	...
	.if eax==WM_INITDIALOG
		...
		; init hex edit
		;~~~~~~~~~~~~~~
		invoke	CreateFontIndirect,addr logfont_err
		mov		hFont_err,eax
		...
		; init grad edit
		;~~~~~~~~~~~~~~~
		invoke	SendDlgItemMessage,hwnd,IDC_GRAD,WM_SETFONT,hFont,TRUE
	...
	@@:
		...
		@@:
			...
			fsub
			; check -1 <= x <= 1
			;invoke	SendDlgItemMessage,hwnd,IDC_GRAD,WM_SETFONT,hFont_err,TRUE 
			fst		temper
			fld		temper
			fld1
			fadd
		...
		@@:
			...
			invoke	SetDlgItemText,hwnd,IDC_GRAD,addr s_f12
...
; CalcParams
;
; When some item of table (with 'g=') was changed
