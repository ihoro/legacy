comment `#####################################################
#
#  ffc.asm    14.03.2006 by fnt0m32 'at' gmail.com
#
#  FF Catcher v0.9.6
#
#  Used [MASM32 v8.2]
...
DlgProc proc uses ebx esi edi hwnd:HWND,msg:UINT,wParam:WPARAM,lParam:LPARAM
	...
	@@:
		...
		mov		ti.uFlags,TTF_IDISHWND or TTF_SUBCLASS
			...
			; FUN button
			;~~~~~~~~~~~
			invoke	GetDlgItem,hwnd,IDC_FUN
			mov		ti.uId,eax
			mov		ti.lpszText,offset ttmFun
			invoke	SendMessage,hTT,TTM_ADDTOOL,NULL,addr ti
	...
	.elseif eax==WM_COMMAND
		...
		@@:
			...
			mov		ovr.hEvent,eax
			invoke	WriteFile,hCOM,addr send_buf,18,addr cSent,addr ovr
		...
		.elseif bx==EN_SETFOCUS && ax >= IDC_U11 && ax <= IDC_U24
			...
			xor		ecx,ecx
			mov		cx,ax
			invoke	SetDlgItemText,hwnd,ecx,addr u_hex
			mov		DontCalcParams,0
		...
		.elseif bx==EN_KILLFOCUS && ax >= IDC_U11 && ax <= IDC_U24
			...
			push	eax
			mov		eax,ecx
			mov		ecx,ParamOfTableSize
			mul		ecx
			lea		esi,[esi+eax]			; get offset of string
			pop		eax
			
			mov		cx,ax
			invoke	SetDlgItemText,hwnd,ecx,esi
		...
		c64_system:
			...
			.if c64_on == 1
				...
				calc_sens2:
					...
					invoke	FloatToStr2,c64_fpu1,addr c64_s1
					invoke	FloatToStr2,c64_fpu2,addr c64_s2
					
					; show new params
					mov		ebx,IDC_U11
					add		ebx,c64_id
					invoke	SetDlgItemText,hwnd,ebx,addr c64_s1		; set sensor #1
					add		ebx,4
					invoke	SetDlgItemText,hwnd,ebx,addr c64_s2		; set sensor #2
...
; then this proc change other params (U0,k,Alpha)
;-------------------------------------------------------------
...
; ExtractKT
;
; Get string from 'kt=' edit and save it to cfg.kt
;-------------------------------------------------------------

ExtractKT proc
	
	invoke	GetDlgItemText,hdlg,IDC_KT,addr checkhex_buf,CheckHexSize
	lea		esi,checkhex_buf
	cld
	xor		ebx,ebx
	mov		ecx,4
@@:
