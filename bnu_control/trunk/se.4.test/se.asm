comment `#####################################################
#
#  se.asm    22.08.05 by fnt0m32 'at' gmail.com
#
#  Script Executor v0.5 (demo version)
#
...
DlgProc proc uses ebx esi edi hwnd:DWORD,msg:UINT,wParam:WPARAM,lParam:LPARAM
	...
	@@:
		...
		lea		esi,[esi+5]
		loop	@B
		invoke	SendDlgItemMessage,hwnd,IDC_PORT,CB_SETCURSEL,1,0
		invoke	SendDlgItemMessage,hwnd,IDC_PORT2,CB_SETCURSEL,0,0
	...
	@@:
		...
		lea		esi,[esi+6]
		loop	@B
		invoke	SendDlgItemMessage,hwnd,IDC_PARITY,CB_SETCURSEL,3,0
	...
	@@:
		...
		push	ebx
				...
				; add my latitude to report as 'llll.ll,N/S,'
				;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				finit
				fld		my_La
				ftst
			...
			conv_La:
				...
				fdiv
				fild	Deg
				fmul				; latitude as degrees
				fstcw	cwr
				or		cwr,0C00h	; RC = 11 (rounding control)
				fldcw	cwr
				fist	ival
				fild	ival
				fsub				; 0.llll as degrees
				mov		Deg,60
				fild	Deg
				fmul				; ll.ll as minutes
				fst		my_La
				
				lea		edi,[outbuf+32]
				invoke	wsprintf,edi,addr fmtLa,ival
				add		edi,2
				mov		esi,edi
			...
			la_set_direction:
				...
				; add my longitude to report as 'yyyyy.yy,E/W,'
				;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				finit
				fld		my_Lo
				ftst
			...
			conv_Lo:
				...
				fdiv
				fild	Deg
				fmul				; longitude as degrees
				fstcw	cwr
				or		cwr,0C00h	; RC = 11 (rounding control)
				fldcw	cwr
				fist	ival
				fild	ival
				fsub				; 0.yyyy as degrees
				mov		Deg,60
				fild	Deg
				fmul				; yy.yy as minutes
				fst		my_Lo
				
				invoke	wsprintf,edi,addr fmtLo,ival
				add		edi,3
				mov		esi,edi
...
@@:
	...
	;~~~~~~~~~~~~~~~~~
		...
		mov		al,tstTime
		mov		byte ptr tstSec,al
		
		; week - don't need, look following code
		;inc		word ptr p88week
		
		; time
		finit
		fld		tbyte ptr p88time
		fild	p88time_add
		fadd
;		fcom	p88time_max
;		fstsw	ax
;		sahf
;		jl		@F
		fstp	tbyte ptr p88time
		fld		tbyte ptr p88time
		fistp	dword ptr time_41
;		jmp		next_one
;	@@:
;		fstp	tbyte ptr p88time
;		fldz
...
;	next_one:
		...
		; send TWO
		;invoke	WriteFile,hCOM_out,addr TWO,TWO_size,addr cGot_tst,addr ovr_out
		;invoke	ResetEvent,ovr_out.hEvent
	.endif
